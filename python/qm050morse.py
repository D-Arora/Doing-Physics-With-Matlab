...
#%%  INPUTS
N = 801                     # x grid points
xMin = 0; xMax = 0.5*sx     # x range [m]
L = 0.5                     # Limit for X axis in plots
...
#axes[2,0].set_xticks(arange(0,0.25+0.05,0.1))  
#axes[2,1].set_xticks(arange(-0,0.25+0.05,0.1))  
fig2.tight_layout()
...
def fig56(yP,q):
   ...
   #ax.set_xticks(arange(0,0.6,0.25)) 
   ax.set_xlim([0,0.3])
   ax.xaxis.grid()
   ax.yaxis.grid()
   ...
   ax.set_xticks(arange(0,0.32,0.1))  

   ax2 = ax.twinx()
   ax2.set_ylabel('U & K [eV]',color= 'k')
   ax2.tick_params(axis='y', labelcolor = 'k')
   ax2.plot(x/sx,Um/se,'r',lw = 1)
   ax2.plot(x/sx,E[n]-Um/se,'g',lw = 1)
   ax2.set_ylim([-20,20])
...
def fig78(q):
    ...
    ax2.plot(x/sx,E[q]-Um/se,'g',lw = 1)
    ax2.set_ylim([-20,20])
    ax2.set_yticks(np.arange(-20,21,5))
...
ax.set_xlim([0,25])
ax.set_ylim([0,10])
ax.set_ylabel('Ep  Em  [ eV ]')
ax.set_xlabel('vibration quantum number')
ax.plot(q,Ew,'bo',ms = 4)
ax.plot(q,ET,'ro',ms = 4)
