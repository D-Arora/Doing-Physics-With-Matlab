...
def equations(variables):
    N,T,I = variables  # Unpack the variables
    eq1 = r2*N*(1-N/K2) - c4*T*N - a3*C*N
    eq2 = r1*T*(1-T/K1) - c2*I*T - c3*T*N - a2*C*T
    eq3 = s + k1*I*T/(k2+T) - c1*I*T - d1*I - a1*C*I
    return [eq1, eq2, eq3]

r2,r1 = 1, 0.98
K2, K1 = 1,1
a1, a2, a3 = 0.2,0.3,0.1
c1,c2,c3,c4 = 1, 0.2, 1, 1
d1, d2 = 0.2,1.0
s = 0.33
k1, k2, k3 = 0.01, 0.3, 1 
V0 = 0
...
IC = [1.0, 1.0, 1.0] # Initial guess for x and y
solution = fsolve(equations, IC)
...
fDotN = r2 - (2*r2/K2)*Nss - c4*Tss - a3*Css
gDotT = r1 - (2*r1/K1)*Tss - c2*Iss - c3*Nss -a2*Css
hDotI = k1*Tss/(k2+Tss) - c1*Tss -d1 - a2*Css

#print(fDotN, gDotT,hDotI)
