...
% DATA: Id Rd Dd / Number of data days for COVID19  
  ...
  Idtot  = covid19(1:Ndays,13);
  Id     = covid19(1:Ndays,14);
  Dd     = covid19(1:Ndays,15);
...
% SURGE FACTORS
  s = zeros(num,1);
  for n = 1:num-1
     if t(n) > 70; s(n) = 0.5; end 
     if t(n) > 77; s(n) = 1; end
     if t(n) > 82; s(n) = 2;end
     if t(n) > 88; s(n) = 3;end
     if t(n) > 94; s(n) = 4;end
     if t(n) > 102; s(n) = 5;end
     if t(n) > 106; s(n) = 6;end
     if t(n) > 120; s(n) = 0; end
  end
