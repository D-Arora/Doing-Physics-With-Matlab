...
for ct = 1 : nt-1
    ...
    for cx = 2 : nx
        T(ct+1,cx) = T(ct,cx) +  K2(cx) * ( J(ct,cx-1) - J(ct,cx)) ;
    end
end
   %if T(:,1) > T(:,end);
       J(end,:) = J(end-1,:);
...
if T(:,1) < T(:,end);
Ylimit = [f*round(2*max(abs((J(end,:))))) 0];
else 
    Ylimit = [0 f*round(2*max(abs((J(end,:)))))];
end
...
tx1 = 'Time to reach seady-state 5 \tau = ';
tx2 = num2str(5*tau,4);
tx3 = '  s';
tx = [tx1 tx2 tx3];
text(10,22,tx);
...
tx1 = 'Number of elements  N  = ';
tx2 = num2str(nx,3);
tx3 = ' ';
tx = [tx1 tx2 tx3];
text(10,10,tx);

tx1 = 'Number of time steps   = ';
tx2 = num2str(nt,5);
tx3 = ' ';
tx = [tx1 tx2 tx3];
text(10,6,tx);
