...
nP = 49;                      % observation points for P
nQ = 55;                      % aperture points for Q  must be ODD

wL = 632.8e-9;               % wavelength [m]
aQ = 10*wL;                   % radius of circular aperture [m]
zP = 1000*wL;                    % z distance between aperture plan and observation plane [m]

xPmin = 0;
xPmax = 10e-5;

nShift = 100;                     % separation of sources
...
EPtotal = EP1 + EP2;
WPtotal = conj(EPtotal) .* EPtotal;
WPmax = max(WPtotal);
WPtotal = WPtotal ./ WPmax;
WPtotals = 10 .* log10(WPtotal);

WPPtotal = meshgrid(WPtotal,WPtotal);
xx1 = xx((nShift : end-1),(nShift : end-1));
yy1 = yy((nShift : end-1),(nShift : end-1));
...
pcolor(xx1,yy1,10.*(WPPtotal).^0.3);
