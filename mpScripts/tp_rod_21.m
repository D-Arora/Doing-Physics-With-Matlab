...
% internal heating - energy flux density
   area = pi * r(1)^2;   % assume uniform cross-sectional area
   Pint = 10;
   Jint = zeros(1,nx);
   Jint(nx/2-10 : nx/2 +10) = Pint/area;
...
for cx = 2 : nx-1
    T(ct+1,cx) = T(ct,cx) + K2(cx) * (Jint(cx) + J(ct+1,cx) - J(ct+1,cx+1) ) ;
end
 %   T(ct+1,1)   = T(ct,1) + K2(1) * J(ct+1,2);
 %   T(ct+1,end) = T(ct,end) + K2(end) * J(ct+1,end-1);
end
    % T(:,1) = T(:,2);
    % T(:,end) = T(:,end-1);
