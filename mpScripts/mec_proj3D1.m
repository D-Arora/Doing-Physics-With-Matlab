...
%  Drag force  D and Lift force L 
   ...
   for c = 1:N
       ...
       Lx(c) = KL*(v/w)*( wy*vz(c) - wz*vy(c) );
       Ly(c) = KL*(v/w)*( wz*vx(c) - wx*vz(c) );
...
%  RESULTS  ==================================================
   ...
   yMax = max(y); xMax = x(N); zMax = max(z);
...
figure(1)
   ...
   set(gcf,'position',[0.06 0.05 0.35 0.20]);
   set(gcf,'color','w')  
 ...
 % xlim = [0 180];
  axis equal
 % ylim([0 40])
  ticks = (0:20:60); yticks(ticks);
  ticks = (0:20:250); xticks(ticks);

figure(2)   % 222222222222222222222222222222222222222222222222222222
  set(gcf,'units','normalized');
  set(gcf,'position',[0.06 0.40 0.35 0.30]);
  set(gcf,'color','w')   

  plot3(x,z,y,'b','linewidth',3)
  ylim([-30 30])
  ax = gca;
  ax.YDir = 'reverse';
  grid on; box on
...
% Displacement vs time 3333333333333333333333333333333333333333333
figure(3)  
  set(gcf,'units','normalized');
  set(gcf,'position',[0.32 0.05 0.35 0.50]);
  set(gcf,'color','w') 
...
% Velocity vs time  4444444444444444444444444444444444444444444444444444
figure(4)  
  set(gcf,'units','normalized');
  set(gcf,'position',[0.52 0.08 0.33 0.66]);
  set(gcf,'color','w') 
...
% Forces vs time  555555555555555555555555555555555555555555555555
figure(5)  
  set(gcf,'units','normalized');
  set(gcf,'position',[0.62 0.1 0.30 0.50]);
  set(gcf,'color','w') 
...
subplot(2,1,1)
  ...
  plot(t,Dz,'m','LineWidth',2)
  txtX = 't  [ s ]';
  txtY = 'Drag / Weight '; 
 % txtT = sprintf('t_F = %2.2f   x_F = %2.2f   y_F = %2.2f   v_F = %2.2f \n',tF, xF, yF, vF);
  txtT = '  ';
  legend('D_x','D_y','D_z','Location','southeast')
  plotSetup(txtX,txtY,txtT)

subplot(2,1,2)
  plot(t,Lx,'b','LineWidth',2)
  hold on
  plot(t,Ly,'r','LineWidth',2)
  plot(t,Lz,'m','LineWidth',2)
  txtX = 't  [ s ]';
  txtY = 'Lift / Weight '; 
 % txtT = sprintf('t_F = %2.2f   x_F = %2.2f   y_F = %2.2f   v_F = %2.2f \n',tF, xF, yF, vF);
  txtT = '  ';
