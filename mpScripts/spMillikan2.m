...
% KNOWN PARAMETERS ======================================================
 ...
 % Assign number of elementary charges to oil drop
 N = ones(30,1);
 N(1:3)   = 6;
 N(4:9)   = 4;
 N(10:14) = 5;
...
% CALCULATIONS ==========================================================
  ...
  % time intervals for rising oil drop: E <> 0   [s]
    tU =  (6*pi*eta*R*s) ./ (N.*(e*E)- m*g);  
    tU = tU + 0.1*randn(30,1);
  ...
  % Average charge per group
    q_avg(1) = mean(q(1:3));
    q_avg(2) = mean(q(4:9));
    q_avg(3) = mean(q(10:14));
    q_avg(4) = mean(q(15:20));
    q_avg(5) = mean(q(21:25));
    q_avg(6) = mean(q(26:30));
...
% Estimate for eleementary charge
 ...
 figure(1)
   ...
   grid on
   set(gca,'YTickLabel',[]);
   xlabel('q x 10^{-19} [C] ','fontsize',fs);
   set(gca,'fontsize',fs);
