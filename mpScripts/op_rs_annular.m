...
% Aperture Space -------------------------------------------------------
zQ = 0;
A = zeros(nR,1);       % integral for each ring in aperture space
n = zeros(nR,1);       % number of points for ring in aperture space
EQ = zeros(nR,1);   % Aperture: Electric Field {V/m]
a1 = f * a;
UQ = uQmax * pi * (a^2 - a1^2);   % Theoretical energy emitted from aperture  [J/s]
...
EQ(r > a1) = sqrt(2*uQmax/(cL*nRI*eps0));   

% double annulus
%EQ(r < a1/2) = sqrt(2*uQmax/(cL*nRI*eps0));
