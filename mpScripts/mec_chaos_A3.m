...
% Time domain:  nT must be an ODD number 
   nT = 3801;
   tMin = 0;
   tMax = 300;
...
% Initialise arrays and set initial conditions    
   xA = zeros(nT,1);
   vA = zeros(nT,1);
...
figure(2)   % x vs  v f energy------------------------------------------
   ...
   set(gcf,'color','w');
% subplot(3,1,1)
   XY = 1:nT;           % specifiy time interval for plotting
   xP = xA(XY)/pi; yP = vA(XY);
   plot(xP,yP,'b','lineWidth',2);
   hold on
   xP = xA(1)/pi; yP = vA(1);
   Hplot = plot(xP,yP,'o');
   set(Hplot,'markersize',6,'markerFaceColor','r','markerEdgeColor','r')
   xP = xA(end)/pi; yP = vA(end);
   Hplot = plot(xP,yP,'o');
