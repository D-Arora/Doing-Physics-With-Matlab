...
disp('   8: Lattice');
disp(' ');
disp('   10: miscellaneous -->');

wellType = input('Specify well type: 1, 2, 3, 4, 5, 6, 7, 8,  10 -->   ');
disp(' ');
disp(' ');
...
% Colvalent bonding   V shaped potentail wells   
   ...
   s = sprintf('Potential Well: Double: V Shaped');
    ...
    case 10
    xMin = -20;         % default = -0.1 nm
    xMax = +20;         % default = +0.1 nm
    x1 = 10;            % 1/2 well width: default = 0.05 nm
    U1 = -0.02;            % Depth of well (eV): default = -400 eV
    U2 = -0.01;
   ...
   for cn = 1 : num
   if x(cn) >= -x1,  U(cn) = U1; end;
   if x(cn) >= 0,    U(cn) = U2; end;
   if x(cn) >= x1, U(cn) = 0; end;  
   end; %for
   s = sprintf('Potential Well: miscellaneous');
