...
% Blackbody spectrum of Stars at different temperatures
...
% Wavelength at peak of Sun's spectral exitance in wavelenght  
  wL_peak = b_wL/T;
...
% Area under curves: power output of Star  [a.u.]
  P_total = simpson1d(R_wL,wL1,wL2); 
  P_vis   = simpson1d(R_wL_vis,wL1_vis,wL2_vis);
  P_IR    = simpson1d(R_wL_IR,wL1_IR,wL2_IR);
  P_UV    = simpson1d(R_wL_UV,wL1_UV,wL2_UV);
...
disp('    ');
disp('    ');
fprintf('Star: temperature of photosphere, T_S = %0.0f  K \n',T); 
disp('    ');
disp('Peak in Star Spectrum');
fprintf('   Theory: Wavelength at peak in spectral exitance, wL = %2.0f  nm  \n',1e9*wL_peak); 
fprintf('   Graph:  Wavelength at peak in spectral exitance, wL = %2.0f   nm  \n',1e9*wL_peak_graph); 
