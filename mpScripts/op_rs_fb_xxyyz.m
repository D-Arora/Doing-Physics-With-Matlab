...
% Source  S
    zS = 0.1;
    xS = 0.01;         % change xS for off-axis source
...
%  nXY:   +X (1)   -X (3)   +Y (2)   -Y (4)
    ...
    vPxp = (2*pi*a/wL) .* (xP(:,nXY(1))-xS) ./ sqrt(xP(:,nXY(1)).^2 + zP^2);  
    vPxm = (2*pi*a/wL) .* (xP(:,nXY(3))-xS) ./ sqrt(xP(:,nXY(3)).^2 + zP^2);  
...
% ========================================================================
  ...
  % optical coordinate
   ...
   for c1 = 1 : nP
   for c2 = 1 : nQR
      rPQ = fn_distancePQ(xA,yA,zA(c1),xQ(c2,:),yQ(c2,:),zQ);
      rPQ3 = rPQ .* rPQ .* rPQ;
      kk = ik .* rPQ;
      MP1 = exp(kk);
      MP1 = MP1 ./ rPQ3;
      ...
      A(c2) = rQ(c2) * simpson1d(fn,0,2*pi);
   end
     EA(c1) = drQ * sum(A)/ (2*pi);
   end
   
   we_A = (cL*nRI*eps0/2) .* abs(EA).^2;
   we_A_max = max(we_A);
   we_A_dB = 10 .* log10(we_A./we_A_max);
...
figure(6)   % Z axis ---------------------------------------------------
 set(gcf,'Units','normalized');
   set(gcf,'Position',[0.3 0.2 0.3 0.3]);
   fs = 12;
   tx = 'optical axial coordinate  u_P / \pi ';
   ty = ' axial irradiance W.m^{-2} ';
   x = uA ./ pi; y = we_A;
