...
%% [case: redundant] Nyquist freq >> Signal freq 
   ...
   oscAliasingPlot(f0,frameRate,nCycles,Nx,Ny,titulo,h);
...
%%
function oscAliasingPlot(freqSignal,frameRate,nCycles,Nx,Ny,titulo,h)
% Generate and show rotating spot, show x-position x(t) of spot center in relation to image center and frequencies
  ...
  for n = 1:nFrames
   ...
   % set(hPlot,'markersize',8,'markerfacecolor','b')
    ...
    xlabel('time [s]'); ylabel('x spot postion [a.u.]');  % 
    title(titulo);
    set(gca,'fontsize',12)
    box on
    ...
    if(fNyq>=freqSignal)
        legend('Signal','Nyquist'); title('Frequency domain: Signal freq and Nyquist freq');
    else      
        ...
        line([f_alias f_alias],[0 1],'Color','g','LineWidth',3); legend('Signal','Nyquist','Alias');
