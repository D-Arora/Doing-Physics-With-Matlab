...
for nt = 1 : Nt-2
  for c = 2 : N-1
    e(1,nt+1) = 0.5*cos(17.05*(nt+1)*dt) + ...
                0.5*cos(14.14*(nt+1)*dt) ;
...
% Natural Frequencies ================================================
  ...
  omega = zeros(modeN,1);
  for c = 1 : modeN
      omega(c) = sqrt( (4.*kS0/m0) * sin(c*pi/(2*N))^2 );
  end
  f_mode = omega./(2*pi);
...
for c = 1: 5 : Nt
 ...
 subplot('position',[0.1 0.8 0.8 0.1])   
    for cm = 1 : N
       xP = (cm*d + e(cm,c)); yP = 0;
       hPlot = plot(xP,yP,'ob');
       set(hPlot,'markersize',3,'markerfacecolor','b')
       hold on
    end
      xlabel('x  [mm]')
      xlim([0 Xmax])
 ...
 subplot('position',[0.1 0.2 0.8 0.4])  
    ...
    for cm = 1 : N
     %  xP = ((1:N).*d + e(1:N,c)); yP = e(:,c);
     %  plot(xP,yP,'b','linewidth',2);  
     %  hold on
       
       xP = (cm*d + e(cm,c)); yP = e(cm,c);
       hPlot = plot(xP,yP,'ob');
    ...
    %   ax.XAxis.MinorTickValues = 0:10:210; 
       grid on;
    %   ax.XMinorGrid = 'on';
