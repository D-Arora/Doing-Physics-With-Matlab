...
% Amyloid-beta peptide: number of molecules  
  ...
  for n = 1: N-1
   ...
   Ab(n+1) = Ab(n) + h*(a(15)*Ns(n) - a(16)*M2(n) - a(17)*Ab(n)) ;
   
%   M3(n+1) = M3(n) + w - b*M1(n) + d*M3(n);
...
figure(1)
   ...
   set(gcf,'position',[0.02 0.10 0.25 0.3]);
   set(gcf,'color','w');
...
%    plot(xP,yP,'g','linewidth',2)
   ...
   legend('N_s', 'M_1', 'A\beta','location','east')
   set(gca,'fontsize',FS,'fontname','times')
  
figure(2)
   set(gcf,'units','normalized');
   set(gcf,'position',[0.32 0.10 0.25 0.3]);
   set(gcf,'color','w');
