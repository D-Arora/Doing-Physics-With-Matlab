...
% ========================================================================
% INPUT PARAMETERS 
num = 200;             % number for observation space
...
zPmin = 1*wL;
zPmax = 600*wL;               % Aperture to Screen distance  [m]
...
for c1 = 1 : nP
       rPQ = fn_distancePQ(xP,yP,zP(c1),xQ,yQ,zQ);
       rPQ3 = rPQ .* rPQ .* rPQ;
       kk = ik .* rPQ;
       MP1 = exp(kk);
       MP1 = MP1 ./ rPQ3;
       ...
       MP = EQ .* MP1 .* MP2;
       EP(c1) = simpson2d(MP,xQmin,xQmax,yQmin,yQmax) ./(2*pi) ;
...
% =======================================================================
% Theortical uP
uPenvelope_p = max(uP) .* (1 + zP ./ sqrt(a^2 + zP.^2)).^2 ./ 4; 
uPenvelope_m = max(uP) .* (1 - zP ./ sqrt(a^2 + zP.^2)).^2 ./ 4;
uPtheory =  max(uP) .* (1 - 2.*zP ./ sqrt(a^2 + zP.^2) .* cos(k .* (sqrt(a^2 + zP.^2)-zP)) + zP.^2 ./(a^2 + zP.^2)) ./ 4;
...
% energy density   uP vs Xp & yP
   ...
   ty = 'energy density  u_P  [W.m^{-2}]';
   x = zP ./ wL; y = uP;
   %plot(x([1:20:end]),y([1:20:end]) ,'bo');
    plot(x,y ,'r');
   xlabel(tx,'Fontsize',fs);   ylabel(ty,'Fontsize',fs);
