...
% Basal glucose and insulin (roots of cubic polynomial)
  p1 = a2+a3*Imax/kI; p2 = -q10; p3 = a2*a; p4 = a3*Imax/kI - q10*a;
  z = roots([p1 p2 p3 p4]);
...
% Variation in glucose production over a day
  ...
  w = 1*60;
  ag10 = 0.3;
  ag1 = ag10.*exp( -(t - tC).^2 /(2.*w^2) );
 % ag1 = a1 + ones(1,NT).*ag1;
  ag1 = q10 + ones(1,NT).*ag1;
...
%   dG_prod(n+1) = dt*a1*5/(1+exp(0.1*(G(n)-G_basal)));
   dG_prod(n) = ag1(n);  
   dG_prod(n) = 2*(ag1(n))/(1+exp(0.01*(G(n)-G_basal)));

%  Glucose decay   
   dG_decay(n) = -a2*G(n);
   
%  Glucose metabolism by insulin
   dG_insulin(n) = - a3*I(n)*G(n);
...
% Insulin  pancrease
  ...
  dI_pancrease = dt * Imaxg(n);
  I_pancrease = I_pancrease + dI_pancrease;
% I_decay
  I_decay =  - dt*kI*I(n);
 ...
 dG_prod(1) = dG_prod(2);
 dG_prod(end) = dG_prod(1);
 dG(NT) = dG_prod(NT-1);
 dG_decay(1) = dG_decay(2);
 dG_decay(NT) = dG_decay(NT-1);
 dG_insulin(1) = dG_insulin(2);
...
I_GNC = (q10 - a2.*G_NC)./(a3.*G_NC);

yP = G_NC; xP = I_GNC;
plot(xP,yP,'m','linewidth',2)

xP = Imax.*(G_NC.^2./(a+G_NC.^2))./kI;
