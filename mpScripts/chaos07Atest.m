...
% ANIMATION SETUP   ===================================================
% flagG = 0  animation noy displayed / flagG = 1 animation displayed
   flagG = 0;
...
% CALCULATIONS ========================================================
  XY = nS:nF;    % time interval plotting range
...
% Runga-Kutta Solution of differential equation
   for cc = 1 : nT-1
       ...
       v(cc+1) = v(cc) + (k1 + 2*k2 + 2*k3 + k4)/6;
        ...
        while x(cc+1) >  pi; x(cc+1) = x(cc+1) - 2*pi; end
        while x(cc+1) < -pi; x(cc+1) = x(cc+1) + 2*pi; end
   end
...
figure(5)   % x vs  v f energy------------------------------------------
   pos = [0.32 0.45 0.29 0.3];
   set(gcf,'Units','normalized');
   set(gcf,'Position',pos);
   set(gcf,'color','w');

            
   xP = x(1:N:nT)/pi; yP = v(1:N:nT);
   hold on
...
% y = - ( c(1)*sin(x) + c(2)*v - 2* c(3)*cos(c(4)*t)*sin(x) ) ;
end
